...
alter table RECOLECTIONS
   drop constraint PK_RECOLECTIONS;
...
alter table SOLIDS
   drop constraint PK_SOLIDS;

drop table SOLIDS;

alter table SOLID_TYPE
   drop constraint PK_SOLID_TYPE;
...
alter table RECOLECTIONS
   add constraint PK_RECOLECTIONS primary key (RECOLECTION_ID);
...
alter table SOLIDS
   add constraint PK_SOLIDS primary key (SOLID_ID);
...
alter table SOLID_TYPE
   add constraint PK_SOLID_TYPE primary key (SOLID_TYPE_ID);
...
create table TRANSACTIONS (
   ...
   TR_DETAIL            VARCHAR(200)         null,
   TR_IP                VARCHAR(40)          null
);

alter table TRANSACTIONS
   add constraint PK_TRANSACTIONS primary key (TR_ID);
...
create table USERS (
   ...
   USER_FIRST_LASTNAME  VARCHAR(30)          null,
   USER_SECOND_LASTNAME VARCHAR(30)          null,
...
alter table ORDERS
   add constraint FK_ORDERS_REFERENCE_USERS foreign key (GENERATOR_ID)
